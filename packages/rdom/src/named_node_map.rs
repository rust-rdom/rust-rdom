...
pub struct NamedNodeMap {
    ...
    pub attribute_list: Vec<Arc<Node>>,

    /// Reference back up to the core element
    /// would be nice to know this is actually an Element (same as above)
    pub element: Weak<Node>,
}

impl NamedNodeMap {
    fn new(context: Weak<Sandbox>, element: Weak<Node>) -> Arc<NamedNodeMap> {
    ...
    fn item(&self, index: usize) -> Option<Arc<Node>> {
